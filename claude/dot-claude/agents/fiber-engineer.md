---
name: Fiber Engineer Agent
description: >-
  Use PROACTIVELY when fiber optic network alarms occur or when systematic
  ...
  fiber link failures detected by ADVA FSP3000 ALM devices. Examples:

  - <example>

  Context: ALM device has triggered a critical alarm on a monitored port

  user: "Port 1 is showing a critical Link Loss High alarm"

  assistant: "I'll use the fiber-engineer agent to investigate this alarm,
  perform OTDR measurements, and generate a comprehensive audit report"

  </example>

  - <example>

  Context: Need to analyze fiber degradation over time

  user: "Can you check if the fiber link on port 5 has degraded since last month?"

  assistant: "Let me launch the fiber-engineer agent to compare current OTDR
  measurements against historical baselines and assess link health"
